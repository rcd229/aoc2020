const groupAnswers = `gwlcpbtxmiezd
xwlcpbtzimgdk
tbiwmpcgzdxo
ygzdbpjxncfwimt
...
].concat(questions.map(q => { return { title: q, field: q } })).concat([
  {
    title: "Anyone Answered",
    field: "anyone"
  },
  {
    title: "All Answered",
...
export const getSums = () => {
  ...
  groups.forEach(group => {
    ...
    answers.forEach(answer => {
      ...
      allAnswers = allAnswers.concat(questions);
    });

    const allQuestions = new Set(allAnswers);
    any += allQuestions.size;

    const allAnswered = getAllAnswered([...allQuestions], allAnswers, answers.length);
    all += allAnswered;
  });
...
const getAllAnswered = (questions, groupAnswers, groupLength) => {
  let sum = 0;

  questions.forEach(question => {
    if (groupAnswers.filter(a => a === question).length === groupLength) {
      sum++;
    };
  });
  return sum;
...
export const processData = () => {
  ...
  groups.forEach((group, i) => {
    ...
    answers.forEach(answer => {
      const questions = answer.split("");
      questions.forEach(q => {
        groupData[q] = groupData[q] ? groupData[q] + 1 : 1;
      });
    });

    const any = Object.keys(groupData);
    const all = any.filter(x => groupData[x] === answers.length);

    groupData.any = any.length;
    groupData.all = all.length;
    groupData.group = `Group ${i}`;
