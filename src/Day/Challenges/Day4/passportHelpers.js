...
const rules = {
  ...
  hgt: {
    rule: /^(1([5-8]\d|9[0-3])cm|(59|6\d|7[0-6])in)$/,
    description: "[150-193]cm or [59-76]in"
  },
  hcl: {
    rule: /^#([0-9]|[a-f]){6}$/,
    description: "'#' followed by exactly six characters 0-9 or a-f"
  },
  ecl: {
    rule: /^(amb|blu|brn|gry|grn|hzl|oth)$/,
...
export const processPassports = () => {
  const passports = passportData.split(/\n\n/);
  const processedPassports = passports.map(passport => {
    return getFields(passport);
  });

  return processedPassports;
};
...
export const checkPassports = passports => {
  let validPs = [];
  let invalidPs = [];

  passports.forEach(passport => {
    const pFields = Object.keys(passport);
    if (pFields.length === reqFields.length
      || (pFields.length === reqFields.length - 1 && !pFields.includes("cid"))) {
      validPs.push(passport);
    } else {
      invalidPs.push(passport);
    }
  });
...
export const checkRules = passports => {
  const validPs = [];

  passports.forEach(passport => {
    if (checkValidity(passport).valid) { validPs.push(passport); }
  });
